(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.BHOCPublications=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function overview(data){return {total:data.length,groups:speciesGroups.map(g=>({...g,count:data.filter(p=>matchesSpecies(p,g.value)).length})),journalLabels:new Set(data.map(p=>normalize(p.journal)).filter(Boolean)).size,institutions:institutionOptions(data).length-2,linked:data.filter(p=>p.pubmed_url).length,dois:data.filter(p=>p.doi).length,unspecified:data.filter(p=>!species(p).length).length};}
  ...
  return {OTHER,normalize,institutions,species,authorOptions,speciesGroups,matchesSpecies,overview,institutionOptions,filter,sort,paginate,safeURL,validate};
});
