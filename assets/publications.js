...
(() => {
  ...
  const fields={q:'keywordInput',author:'authorInput',year:'yearSelect',journal:'journalInput',species:'speciesInput',scope:'scopeSelect'};
  ...
  function card(p){
    ...
    return `<article class="record"><div class="record-top"><span class="tag year">${esc(p.year)}</span>${C.species(p).map(s=>`<span class="tag">${esc(s)}</span>`).join('')}${type?`<span class="tag">${esc(type)}</span>`:''}</div><h3>${C.safeURL(source)?`<a href="${esc(C.safeURL(source))}" target="_blank" rel="noopener noreferrer">${esc(p.title)}</a>`:esc(p.title)}</h3><p class="authors">${esc(p.source_authors||p.authors)}</p><p class="journal">${esc(p.journal)}</p><div class="record-bottom"><div class="institution-label"><strong>Author affiliation:</strong> ${esc(institutionText)}</div><div class="source-links">${lookup}</div></div><details><summary>Source &amp; affiliation details</summary><p>${citationStatus}</p>${p.source_review_note?`<p>${esc(p.source_review_note)}</p>`:''}${p.source_title&&p.source_title!==p.title?`<p><strong>Source title:</strong> ${esc(p.source_title)}</p>`:''}${p.supplied_year?`<p>Indexed publication year: ${esc(p.year)}. The supplied bibliography used ${esc(p.supplied_year)}; online and print dates may differ.</p>`:''}${(p.affiliations||[]).map(a=>`<p>${esc(a)}</p>`).join('')}${p.affiliation_source_url?`<p>${link(p.affiliation_source_url,'Affiliation evidence')} · ${esc(p.affiliation_source_type)} · ${esc(p.affiliation_source_locator)}.</p>`:''}${!(p.affiliations||[]).length?'<p>No institutional affiliation has been established from the available article-level sources.</p>':''}<p>Species labels: ${esc(p.species_basis)}. ${esc(p.species_correction_note||'')} Institutions are author affiliations, not endorsements or a complete list of study sites.</p></details></article>`;
  }
  ...
  ['yearSelect','speciesInput','scopeSelect','journalInput','sortSelect','pageSize'].forEach(id=>$(id).addEventListener('change',()=>{page=1;render();}));
